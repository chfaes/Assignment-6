...
class Employee(object):
    ...
    def __str__(self):
        return str(self.get_name()) + " is of type " + str(self.get_title())
...
class Manager(Employee):
    ...
    def get_sales_report(self,salesman):
        sum = 0
        for i in Salesman.sales:
            if i == salesman:
                sum += Salesman.sales[i]
        if salesman not in Salesman.sales.keys():
            raise KeyError("Salesman " + str(salesman.get_name()) + " has not sold anything yet.")
        print(str(salesman.get_name()) + "'s current cumulative sales: " + str(sum))
...
class Salesman(Employee):
    ...
    def sale(self,vehicle,sales_price,customer):
        if customer.get_score:
            ...
            print(str(customer) + " does not have enough credit score!")
